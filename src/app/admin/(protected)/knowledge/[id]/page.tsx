...
export default function KnowledgeForm({ params }: { params: Promise<{ id: string }> }) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const url = isNew ? "/api/admin/knowledge" : `/api/admin/knowledge/${id}`;
  ...
  return (
    <div className="max-w-4xl">
      <div className="flex justify-between items-center mb-10">
        <h1 className="text-3xl font-display text-[var(--navy)] tracking-wide">{isNew ? "New Knowledge Article" : "Edit Article"}</h1>
        <Link href="/admin/knowledge" className="text-[var(--navy)]/60 hover:text-[var(--navy)] text-sm font-medium uppercase tracking-widest font-body transition-colors">
          Cancel
        </Link>
      </div>
